...
static int cluster_num = MAX_TYPE;
		...
		const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &val) > 0) {
		if (val < info[type].min_freq_info) {
			val = info[type].min_freq_info;
		} else if (val > info[type].max_freq_info) {
			val = info[type].max_freq_info;
		}
	}
	...
	if (!policy) {
		pr_warn("%s: Fail to get cpu%d policy\n", __func__, rcpu);
		ret = -EPERM;
		goto failure;
	}
