...
#endif
#else
.MACRO TRANSLATE_MPIDR_TO_CPU_INDEX_MAP result, mpidr, anchor, table, data

ldr	\result, =__cpu_logical_map_phys
adr	\table, \anchor
add	\result, \result, \table
ldr	\table, =\anchor
sub	\result, \result, \table			
ldr	\table, [\result]				
