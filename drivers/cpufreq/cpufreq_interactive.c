...
#ifdef CONFIG_CPU_INTERACTIVE_CLUSTER_SYNC
unsigned int big_sync_freq = 0;
module_param(big_sync_freq, uint, 0644);
unsigned int little_follow_freq = 0;
module_param(little_follow_freq, uint, 0644);
#endif
...
static int cpufreq_interactive_speedchange_task(void *data)
{
...
#ifdef CONFIG_CPU_INTERACTIVE_CLUSTER_SYNC
	int boost_little = 0;
	cpumask_t little_online_mask;
	cpumask_t big_online_mask;
	...
	*big_mask.bits = CONFIG_PERFORMANCE_CLUSTER_CPU_MASK;
	pr_debug("CPU interactive cluster Lit_mask=%x big_mask=%x\n",
		CONFIG_POWER_CLUSTER_CPU_MASK, CONFIG_PERFORMANCE_CLUSTER_CPU_MASK);
#endif
	...
	while (1) {
...
#ifdef CONFIG_CPU_INTERACTIVE_CLUSTER_SYNC
		boost_little = 0;
		cpumask_and(&big_online_mask, cpu_online_mask, &big_mask);
		for_each_cpu(i, &big_online_mask){
			pcpu = &per_cpu(cpuinfo, i);
			if(pcpu->target_freq >  big_sync_freq || pcpu->policy->cur > big_sync_freq){
				boost_little = 1;
				break;
			}
		}
		...
		if(boost_little){
			cpumask_and(&little_online_mask, cpu_online_mask, &little_mask);
			i = cpumask_first(&little_online_mask);
...
#ifdef CONFIG_CPU_INTERACTIVE_CLUSTER_SYNC
			if(likely(big_sync_freq > 0 && little_follow_freq > 0)){
				if(cpumask_test_cpu(cpu, &little_mask) && boost_little)
					if(max_freq < little_follow_freq)
						max_freq = little_follow_freq;
			}
#endif
