...
#define MAX_MSM_RPM_WAIT_RETRY	(10)
		...
		if (0 == remain) {
			...
			if (retry == 0) {
				...
				WARN(1, "max %u msecs timeout for waiting msg rpm ack of msg %u.\n",
						SMD_CHANNEL_NOTIF_TIMEOUT * MAX_MSM_RPM_WAIT_RETRY, msg_id);
				...
				pr_warn("%s: %u msecs timeout for waiting msg rpm ack of msg %u. Poll SMD channel.\n",
						__func__, SMD_CHANNEL_NOTIF_TIMEOUT, msg_id);
...
static int msm_rpm_dev_probe(struct platform_device *pdev)
{
	...
	static int rpm_smd_channel_retry_count = 3;
	...
	ret = smd_named_open_on_edge(msm_rpm_data.ch_name,
				...
				msm_rpm_notify);
	if (ret) {
		if (rpm_smd_channel_retry_count > 0 && ret == -ENODEV) {
			pr_warn("%s: Cannot open RPM channel %s %d, defer probe to retry (%d)\n",
				__func__, msm_rpm_data.ch_name,
				msm_rpm_data.ch_type, rpm_smd_channel_retry_count);
			rpm_smd_channel_retry_count--;

			ret = -EPROBE_DEFER;
		}

		if (ret != -EPROBE_DEFER) {
			pr_err("%s: Cannot open RPM channel %s %d err %d\n",
				__func__, msm_rpm_data.ch_name,
				msm_rpm_data.ch_type, ret);
			BUG();
		}
		goto fail;
	}
