...
void dispatch_event(unsigned long code, uint16_t proc)
{
	...
	if (apr_client->svc_cnt > 0) {
		pr_err("%s, APR_CLIENT_AUDIO svc_cnt should be Zero, %d\n", __func__, apr_client->svc_cnt);
	}
	...
	if (apr_client->svc_cnt > 0) {
		pr_err("%s, APR_CLIENT_VOICE svc_cnt should be Zero, %d\n", __func__, apr_client->svc_cnt);
	}
}
