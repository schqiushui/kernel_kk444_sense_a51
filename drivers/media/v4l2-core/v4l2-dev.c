...
EXPORT_SYMBOL(video_device);





int htc_check_video_devArray(void)
{
	int i;
	for(i=0; i<VIDEO_NUM_DEVICES; i++)
	{
		if( video_device[i] && ((unsigned int)video_device[i]->dev.kobj.entry.next == 0x1)){
			panic("[v412][%s] memory corruption detected!", video_device[i]->name);
		}
	}
