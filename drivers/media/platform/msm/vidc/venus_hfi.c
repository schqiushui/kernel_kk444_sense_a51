...
static int venus_hfi_iommu_attach(struct venus_hfi_device *device)
{
	...
	dprintk(VIDC_WARN,"venus_hfi_iommu_attach +\n");
		...
		struct bus_info *bus, int load)
{
	...
	dprintk(VIDC_WARN, "Required bus = %d\n", j);
	return j;
}
...
static inline int venus_hfi_power_on(struct venus_hfi_device *device)
{
	...
	dprintk(VIDC_WARN, "Before regulator_enable\n");
	...
	dprintk(VIDC_WARN, "After regulator_enable\n");
	if (rc) {
...
static int venus_hfi_scale_clocks(void *dev, int load, int codecs_enabled)
{
	...
	struct venus_hfi_device *device = dev;
	struct clock_info *cl;
	dprintk(VIDC_INFO, "venus_hfi_scale_clocks +\n");
	if (!device) {
...
static int venus_hfi_load_fw(void *dev)
{
	int rc = 0;
	struct venus_hfi_device *device = dev;
	dprintk(VIDC_WARN, "venus_hfi_load_fw +\n");
	if (!device) {
	...
	if (!device->resources.fw.cookie) {
		dprintk(VIDC_WARN, "Before loading venus\n");
		device->resources.fw.cookie = subsystem_get("venus");
		dprintk(VIDC_WARN, "After loading venus\n");
	}
