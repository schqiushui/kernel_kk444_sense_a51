...
#define WAKEUPSOURCE_TIMEOUT (2000) 
...
#define D_STATUS_DEBUG(x...) do {} while (0)
...
int smd_pkt_open(struct inode *inode, struct file *file)
{
	...
	D_STATUS_DEBUG("Begin %s on smd_pkt_dev id:%d\n", __func__, smd_pkt_devp->i);
	...
	if (smd_pkt_devp->ch == 0) {
		...
		if (r < 0) {
			...
			D_STATUS_DEBUG("Finished %s on smd_pkt_dev id:%d\n",
				 __func__, smd_pkt_devp->i);
		}
	} else {
		smd_pkt_devp->ref_cnt++;
...
static int smd_pkt_devicetree_init(struct platform_device *pdev)
{
	...
	for_each_child_of_node(pdev->dev.of_node, node) {
		...
		D_STATUS_DEBUG("%s:smd_pkt_init_add_device+, n=[%s]\n", __func__, smd_pkt_devp->dev_name);
		ret = smd_pkt_init_add_device(smd_pkt_devp, i);
		D_STATUS_DEBUG("%s:smd_pkt_init_add_device-, r=[%d]\n", __func__, ret);
		if (ret < 0) {
			pr_err("add device failed for idx:%d ret=%d\n", i, ret);
			kfree(smd_pkt_devp);
			goto error_destroy;
	...
	D_STATUS_DEBUG("SMD Packet Port Driver Initialized.\n");
	return 0;
...
static int msm_smd_pkt_probe(struct platform_device *pdev)
{
	...
	D_STATUS_DEBUG("%s smdpkt_probe_done = %d\n", __func__, smdpkt_probe_done);

	if (pdev) {
		if (pdev->dev.of_node) {
			D_STATUS_DEBUG("%s device tree implementation\n", __func__);
